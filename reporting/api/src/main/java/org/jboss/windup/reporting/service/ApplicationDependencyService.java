...
public class ApplicationDependencyService extends GraphService<ArchiveModel> {
	...
	private Map<String, ApplicationDependencyGraphDTO> appDeps = new HashMap<>();

	public ApplicationDependencyService(GraphContext context) {
		super(context, ArchiveModel.class);
	}

	public Map<String, ApplicationDependencyGraphDTO> getAllDependenciesGraphData() {
		if (MapUtils.isEmpty(appDeps)) {
			...
			.getInputPaths().parallelStream().forEach( inputPath -> {
				...
				appDeps.computeIfAbsent(rootProjectModel.getRootFileModel().getSHA1Hash(), value -> new ApplicationDependencyGraphDTO(rootProjectModel, false, false));
	...
	public Map<String, ApplicationDependencyGraphDTO> getDependenciesGraphDataByInputApp(final ProjectModel projectModel) {
		...
		// first get all entries in the map which are related to this entry
		 Map<String, ApplicationDependencyGraphDTO> workingSet = new HashMap<>();
	...
	private void addChildrenSelected(final ProjectModel projectModel, Map<String, ApplicationDependencyGraphDTO> appDepsSingleApp) {
	...
	private void addChildren(ProjectModel projectModel) {
		...
		children.parallelStream().forEach(item -> {
			...
			final ApplicationDependencyGraphDTO dto = appDeps.computeIfAbsent(item.getRootFileModel().getSHA1Hash(),
					value -> new ApplicationDependencyGraphDTO(item, true, isIgnored));
